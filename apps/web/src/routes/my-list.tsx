...
import { startLibraryImport, watchLibraryImport } from "../api"
...
export const Route = createFileRoute("/my-list")({
  validateSearch: (
    search
  ): { status: "all" | LibraryStatus; sort: LibrarySort; page: number } => ({
    status:
      typeof search.status === "string" &&
      statuses.some(({ value }) => value === search.status)
        ? (search.status as "all" | LibraryStatus)
        : "all",
    sort:
      typeof search.sort === "string" &&
      sorts.includes(search.sort as LibrarySort)
